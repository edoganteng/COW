...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::COW);
    ...
    if (!settings.contains("nAnonymizeCowryAmount"))
        settings.setValue("nAnonymizeCowryAmount", 1000);

    nAnonymizeCowryAmount = settings.value("nAnonymizeCowryAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCowryAmount"))
        SoftSetArg("-anonymizecowryamount", settings.value("nAnonymizeCowryAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cowry.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCowryAmount:
            return QVariant(nAnonymizeCowryAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCowryAmount:
            nAnonymizeCowryAmount = value.toInt();
            settings.setValue("nAnonymizeCowryAmount", nAnonymizeCowryAmount);
            emit anonymizeCowryAmountChanged(nAnonymizeCowryAmount);
            break;
        case CoinControlFeatures:
