...
CzCOWWallet::CzCOWWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcow seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCOWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCOWSeed_deprecated()) {
                    LogPrintf("%s: Updated zCOW seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcow seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcow wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCOWWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCOWCount(nCountLastUsed);
    else if (!walletdb.ReadZCOWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCOWWallet::Lock()
{
    seedMaster = 0;
}

void CzCOWWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCOWWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCOW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCOWWallet::LoadMintPoolFromDB()
{
...
void CzCOWWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCOWWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCOWWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcowTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCOWWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCOW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcowTracker which also adds to database
    pwalletMain->zcowTracker->Add(dMint, true);
...
void CzCOWWallet::SeedToZCOW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCOWWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCOWWallet::GenerateDeterministicZCOW(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCOWWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCOWWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
