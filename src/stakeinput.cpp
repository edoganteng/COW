...
// The zCOW block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCOW is a hash of the serial
...
{
    //Create an output returning the zCOW that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCOWOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCOW output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCOW", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCOWOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCOW output", __func__);
...
{
    CzCOWTracker* zcowTracker = pwallet->zcowTracker.get();
    CMintMeta meta;
    if (!zcowTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcowTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a COW stake is the outpoint
