...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cowry" is a composite category enabling all Cowry-related debug output
            if (ptrCategory->count(string("cowry"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Cowry
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Cowry
// Mac: ~/Library/Application Support/Cowry
// Unix: ~/.cowry
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cowry";
#else
    fs::path pathRet;
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cowry.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cowryd.pid"));
