...
#ifndef Cowry_ZCOWWALLET_H
#define Cowry_ZCOWWALLET_H
...
public:
    CzCOWWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCOW(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCOW(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Cowry_ZCOWWALLET_H
