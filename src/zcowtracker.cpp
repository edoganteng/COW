...
CzCOWTracker::CzCOWTracker(std::string strWalletFile)
{
...
CzCOWTracker::~CzCOWTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCOWTracker::Init()
{
...
bool CzCOWTracker::Archive(CMintMeta& meta)
{
...
bool CzCOWTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCOWTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCOWTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCOWTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCOWTracker::GetSerialHashes()
{
...
CAmount CzCOWTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCOWTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCOWTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCOWTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCOWTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCOWTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCOWTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCOWTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCOWTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCOWTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCOWTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCOWTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCOWTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCOWTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCOWTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCOWTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCOWTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcow from DB\n", __func__, listDeterministicDB.size());
    }
