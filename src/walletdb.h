...
public:
    ...
    bool WriteZCOWSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCOWSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCOWSeed_deprecated(uint256& seed);
    bool EraseZCOWSeed();
    bool EraseZCOWSeed_deprecated();

    bool WriteZCOWCount(const uint32_t& nCount);
    bool ReadZCOWCount(uint32_t& nCount);
